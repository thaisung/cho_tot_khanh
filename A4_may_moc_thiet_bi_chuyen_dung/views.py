...
class Items_ListCreateAPIView(generics.ListCreateAPIView):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    filterset_fields = ['id','User__username','Map','Location__Name','Address__Name','Usage_status__Name','Free_giveaway',
                        'Price','Title','Detailed_description','Seller_information__Name']
    search_fields = ['id','User__username','Map','Location__Name','Address__Name','Usage_status__Name','Free_giveaway',
                        'Price','Title','Detailed_description','Seller_information__Name']
    ...
    def create(self, request, *args, **kwargs):
        ...
        images_A4_data = request.data.pop('images_A4_data', [])
        serializer = self.get_serializer(data=request.data)
        if serializer.is_valid():
            ...
            item_instance = serializer.save(User=request.user, Location=location, Address=address,
                                            Usage_status=usage_status,Seller_information=seller_information,
                                            Guarantee=guarantee)

            for image_data in images_A4_data:
...
class Items_RetrieveUpdateDestroyAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        has_new_images = 'images_A4_data' in request.data and request.data['images_A4_data']
