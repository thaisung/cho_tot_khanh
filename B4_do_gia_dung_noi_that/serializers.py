...
class B4CategorySerializer(serializers.ModelSerializer):
    # ParentCategory = ParentCategory_Serializer(read_only=True)
    class Meta:
        model = Category
        fields = '__all__'

class B4UsageStatusSerializer(serializers.ModelSerializer):
    class Meta:
        model = Usage_status
        fields = '__all__'

class B4GuaranteeSerializer(serializers.ModelSerializer):
    class Meta:
        model = Guarantee
        fields = '__all__'

class B4SellerInformationSerializer(serializers.ModelSerializer):
    class Meta:
...
class B4Items_Serializer(serializers.ModelSerializer):
    ...
    images_A3 = B4Items_image_Serializer(many=True, read_only=True, source='Items_image_Items_B4')
    ...
    Category = B4CategorySerializer(read_only=True)
    Usage_status = B4UsageStatusSerializer(read_only=True)
    Seller_information = B4SellerInformationSerializer(read_only=True)
